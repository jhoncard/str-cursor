...
function safeNextPath(raw: string | null): string {
  if (!raw || !raw.startsWith("/admin") || raw === "/admin/login") {
    return "/admin";
  }
  return raw;
}
...
export default function AdminLoginForm() {
  ...
  async function handleOAuth(provider: "google" | "apple") {
    setAuthError(null);
    setLoading(provider);
    const next = encodeURIComponent(nextAfterAuth);
    ...
    <main className="flex items-center justify-center px-4 py-16 sm:py-24">
      <div className="w-full max-w-md bg-white rounded-2xl p-8 sm:p-10 shadow-sm border border-gray-100">
        <div className="flex justify-center mb-4">
          <div className="w-12 h-12 rounded-2xl bg-[#2b2b36] flex items-center justify-center">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-gray-500 text-sm">
            Use the account that has been granted admin access in Supabase.
          </p>
        </div>
              ...
              <>
                ...
                authError.toLowerCase().includes("not enabled") ? (
                  <p className="text-xs text-red-600/90 leading-relaxed">
                    In Supabase → Authentication → Providers → Google: turn on,
                    add OAuth Client ID and Secret from Google Cloud (Web
                    application), Save. Redirect URI in Google must be{" "}
                    <code className="rounded bg-red-100/80 px-1 break-all">
                      {process.env.NEXT_PUBLIC_SUPABASE_URL
                        ? `${process.env.NEXT_PUBLIC_SUPABASE_URL}/auth/v1/callback`
                        : "YOUR_PROJECT.supabase.co/auth/v1/callback"}
                    </code>
                    . Allowlist in Supabase URL Configuration:{" "}
                    <code className="rounded bg-red-100/80 px-1 break-all">
        ...
        <p className="mt-8 text-center text-xs text-gray-400 leading-relaxed">
          Admin access is assigned in your database. Guest booking?{" "}
          <Link
            href="/register"
            className="text-[#2b2b36] font-medium underline underline-offset-2 hover:text-[#363645]"
          >
